...
class HTML2PDF_parsingHtml
{
    ...
    protected    int $_num      = 0;         // table number
    protected    array $_level;         // table level
    ...
    public function parse()
    {
        ...
        for ($k=0; $k<$nb; $k++) {
            ...
            if ($actions[$k]['name']=='write') {
                ...
                if ((string) $actions[$k]['param']['txt'] === '')
                    unset($actions[$k]);
            }
        }

        // if we are not on the level 0 => HTML validator ERROR
        if ($parents !== []) throw new HTML2PDF_exception(5, $parents);
     ...
     * @access  protected
     */
    protected function _prepareTxt($txt, $spaces = true): string
    {
        if ($spaces) $txt = preg_replace('/\s+/is', ' ', $txt);
        $txt = str_replace('&euro;', '€', $txt);
        return html_entity_decode($txt, ENT_QUOTES, $this->_encoding);
    }
    ...
    protected function _analiseCode($code)
    {
        ...
        foreach ($param as $key => $val) {
            ...
            switch($key)
            {
                ...
                case 'border':
                    unset($param[$key]);
                    if (preg_match('/^\d+$/isU', $val)) $val .= 'px';
                    $border = $val;
                    break;

                case 'cellpadding':
                case 'cellspacing':
                    if (preg_match('/^(\d+)$/isU', $val)) $param[$key] = $val.'px';
                    break;
        ...
        if ($border!==null) {
            $border = $border ? 'border: solid '.$border.' '.$color : 'border: none';
     ...
     */
    public function getHtmlErrorCode($pos, $before=30, $after=40): string
    {
