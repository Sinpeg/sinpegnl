...
class HTML2PDF_myPdf extends TCPDF
{
    ...
    public function Footer()
    {
        ...
        if ($this->_footerParam['date'] && $this->_footerParam['hour']) {
            $txt.= ($txt !== '' && $txt !== '0' ? ' - ' : '').(HTML2PDF_locale::get('pdf03'));
        }
        if ($this->_footerParam['date'] && !$this->_footerParam['hour']) {
            $txt.= ($txt !== '' && $txt !== '0' ? ' - ' : '').(HTML2PDF_locale::get('pdf01'));
        }
        if (!$this->_footerParam['date'] && $this->_footerParam['hour']) {
            $txt.= ($txt !== '' && $txt !== '0' ? ' - ' : '').(HTML2PDF_locale::get('pdf02'));
        }
        if ($this->_footerParam['page']) {
            $txt.= ($txt !== '' && $txt !== '0' ? ' - ' : '').(HTML2PDF_locale::get('pdf04'));
        }
     ...
     * @access public
     */
    public function isLoadedFont($fontKey): bool
    {
        if (isset($this->fonts[$fontKey])) {
            return true;
        }
        return isset($this->CoreFonts[$fontKey]);
    }
     ...
     * @access public
     */
    public function ptTransform(&$x,  &$y, $trans=true): bool
    {
        // load the last Transfomation Matrix
        $nb = count($this->_transf);
        $m = $nb !== 0 ? $this->_transf[$nb-1] : array(1,0,0,1,0,0);
    ...
    public function myBarcode($code, $type, $x, $y, $w, $h, $labelFontsize, $color)
    {
        ...
        if ($labelFontsize !== 0) $h+= ($labelFontsize);
