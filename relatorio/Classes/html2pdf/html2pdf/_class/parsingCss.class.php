    ...
    */
        ...
        foreach ($styles as $nom => $val) {
            switch($nom)
            {
                case 'font-family':
                    $val = explode(',', $val);
                    $val = trim($val[0]);
                    if ($val !== '' && $val !== '0') $this->value['font-family'] = $val;
                    break;
                ...
                case 'font-size':
                    ...
                    if ($val !== 0.0) $this->value['font-size'] = $val;
                    break;
                ...
                case 'border-top-width':
                    ...
                    if ($val !== 0.0) $this->value['border']['t']['width'] = $val;
                    break;

                case 'border-right-width':
                    $val = $this->convertToMM($val, 0);
                    if ($val !== 0.0) $this->value['border']['r']['width'] = $val;
                    break;

                case 'border-bottom-width':
                    $val = $this->convertToMM($val, 0);
                    if ($val !== 0.0) $this->value['border']['b']['width'] = $val;
                    break;

                case 'border-left-width':
                    $val = $this->convertToMM($val, 0);
                    if ($val !== 0.0) $this->value['border']['l']['width'] = $val;
                    break;
    ...
    protected function _getFromCSS()
    {
        ...
        if ($getit !== []) {
            ...
            asort($getit);
            foreach (array_keys($getit) as $key) $styles = array_merge($styles, $this->css[$key]);
        }
     ...
     * @return boolean
     */
    protected function _getReccursiveStyle($key, $lst, $next = null): bool
    {
        // if next step
        if ($next!==null) {
            // we remove this step
            if ($next !== '' && $next !== '0') $key = trim(substr($key, 0, -strlen($next)));
            array_shift($lst);
    ...
    public function convertBackgroundPosition($css, &$res)
    {
        ...
        if ($css[0]=='left') {
            ...
            $x = $css[0];
        } elseif ($this->convertToMM($css[0]) !== 0.0) {
        ...
        if ($css[1]=='left') {
            ...
            $y = $css[1];
        } elseif ($this->convertToMM($css[1]) !== 0.0) {
    ...
    protected function _analyseStyle(&$code)
    {
        ...
        for ($k=0; $k<$itemsCount; $k++) {
            ...
            foreach ($names as $name) {
                ...
                // save the value
                $this->css[$name] = isset($this->css[$name]) ? array_merge($this->css[$name], $css) : $css;
