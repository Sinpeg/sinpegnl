...
class PclZip
{
    ...
    // --------------------------------------------------------------------------------
    public function privCheckFormat($p_level = 0): bool
    {
    ...
    public function privFileDescrExpand(&$p_filedescr_list, &$p_options)
    {
        ...
        // ----- Look each entry
        foreach ($p_filedescr_list as $i => $singleP_filedescr_list) {
            // ----- Get filedescr
            $v_descr = $singleP_filedescr_list;
            // ----- Reduce the filename
    ...
    // Return Values :
    // --------------------------------------------------------------------------------
    public function privWriteFileHeader(&$p_header): int
    {
    ...
    // Return Values :
    // --------------------------------------------------------------------------------
    public function privWriteCentralFileHeader(&$p_header): int
    {
        // TBC
    ...
    // Return Values :
    // --------------------------------------------------------------------------------
    public function privConvertHeader2FileInfo($p_header, &$p_info): int
    {
    ...
    //     0 on error;
    // --------------------------------------------------------------------------------
    public function privCheckFileHeaders(&$p_local_header, &$p_central_header): int
    {
    ...
    public function privDeleteByRule(&$p_result_list, &$p_options)
    {
        ...
        if ($v_nb_extracted > 0) {
            ...
            foreach ($v_header_list as $i => $singleV_header_list) {
                ...
                @rewind($this->zip_fd);
                if (@fseek($this->zip_fd, $singleV_header_list['offset'])) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                ...
                if ($this->privCheckFileHeaders($v_local_header, $singleV_header_list) != 1) {
                    // TBC
                }
                unset($v_local_header);
                // ----- Write the file header
                if (($v_result = $v_temp_zip->privWriteFileHeader($singleV_header_list)) != 1) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                    $v_temp_zip->privCloseFd();
                    @unlink($v_zip_temp_name);
                ...
                if (($v_result = PclZipUtilCopyBlock($this->zip_fd, $v_temp_zip->zip_fd, $singleV_header_list['compressed_size'])) != 1) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                    $v_temp_zip->privCloseFd();
                    @unlink($v_zip_temp_name);
