...
if($unidade == "todas"){ //Caso a unidade selecionada seja todas.
	...
	$objPHPExcel->getActiveSheet()->setCellValue("E2", "Usuários treinados em programas de capacitação");
	$objPHPExcel->getActiveSheet()->setCellValue("F2", "Títulos do acervo de periódicos impressos");
	$objPHPExcel->getActiveSheet()->setCellValue("G2", "Títulos do acervo de livros impressos");
	$objPHPExcel->getActiveSheet()->setCellValue("H2", "Títulos de outros materiais impressos");
	$objPHPExcel->getActiveSheet()->setCellValue("I2", "Títulos do acervo de periódicos eletrônicos");
	$objPHPExcel->getActiveSheet()->setCellValue("J2", "Títulos do acervo de livro eletrônico");
	...
	foreach($row as $r){
        ...
        $objPHPExcel->getActiveSheet()->setCellValue("E".$line1, $r['nUsuariosTpc']);
        $objPHPExcel->getActiveSheet()->setCellValue("F".$line1, $r['tapi']);
        $objPHPExcel->getActiveSheet()->setCellValue("G".$line1, $r['tali']);
        $objPHPExcel->getActiveSheet()->setCellValue("H".$line1, $r['tomi']);
        $objPHPExcel->getActiveSheet()->setCellValue("I".$line1, $r['tape']);
        $objPHPExcel->getActiveSheet()->setCellValue("J".$line1, $r['tale']);
	...
	// Bibliotecas não preenchidas
	
	$linepreenc=$line1+2;
	
	$sheet->getStyle('A'.$linepreenc.':'.'A100')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
	$sheet->getStyle('A'.$linepreenc.':'.'A100')->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_TOP);
	$sheet->setCellValue('A'.$linepreenc,'Bibliotecas não preenchidas');
	$sheet->mergeCells('A'.$linepreenc.':'.'AA'.$linepreenc);
	$sheet->getStyle('A'.$linepreenc.':'.'AA'.$linepreenc)->getFont()->setBold(true)->setName('Arial');
	
	
	$row = $be->buscaBibliNaoPreenchidas($ano);
	
	foreach($row as $r)
	{		
		$linepreenc++;
		$objPHPExcel->getActiveSheet()->setCellValue("A".$linepreenc, $r['nome']);
	}
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A6",  "Usuários treinados em programas de capacitação");
	$objPHPExcel->getActiveSheet()->setCellValue("A7",  "Títulos do acervo de periódicos impressos");
	$objPHPExcel->getActiveSheet()->setCellValue("A8",  "Títulos do acervo de livros impressos");
	$objPHPExcel->getActiveSheet()->setCellValue("A9",  "Títulos de outros materiais impressos");
	$objPHPExcel->getActiveSheet()->setCellValue("A10", "Títulos do acervo de periódicos eletrônicos");
	$objPHPExcel->getActiveSheet()->setCellValue("A11", "Títulos do acervo de livro eletrônico");	
	...
	if($ano<$ano1){ //Relatorio para um periodo de tempo.
		...
		foreach($row as $r){
			...
			$objPHPExcel->getActiveSheet()->setCellValue($column."6", $r['nUsuariosTpc']);
			$objPHPExcel->getActiveSheet()->setCellValue($column."7", $r['tapi']);
			$objPHPExcel->getActiveSheet()->setCellValue($column."8", $r['tali']);
			$objPHPExcel->getActiveSheet()->setCellValue($column."9", $r['tomi']);
			$objPHPExcel->getActiveSheet()->setCellValue($column."10", $r['tape']);
			$objPHPExcel->getActiveSheet()->setCellValue($column."11", $r['tale']);
		...
		foreach($row as $r){
			...
			$objPHPExcel->getActiveSheet()->setCellValue("B6", $r['nUsuariosTpc']);
			$objPHPExcel->getActiveSheet()->setCellValue("B7", $r['tapi']);
			$objPHPExcel->getActiveSheet()->setCellValue("B8", $r['tali']);
			$objPHPExcel->getActiveSheet()->setCellValue("B9", $r['tomi']);
			$objPHPExcel->getActiveSheet()->setCellValue("B10",$r['tape']);
			$objPHPExcel->getActiveSheet()->setCellValue("B11",$r['tale']);
