...
$sheet = new PHPExcel_Worksheet($objPHPExcel, "Infraestrutura");
$objPHPExcel->addSheet($sheet, 0);
$objPHPExcel->setActiveSheetIndex(0);
$sheet->fromArray($colunas);
?>
<?php
$pColumn1 = 0;
$pRow1 = 0;
...
foreach ($dados as $key => $value) {
 ...
 $sheet->setCellValueByColumnAndRow($pColumn1, $pRow1, $nomeUnid);
 $sheet->getStyleByColumnAndRow($pColumn1, $pRow1)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_PATTERN_DARKGRAY);
 foreach ($value as $key1 => $value1) {
 foreach ($value1 as $value2) {
 foreach ($value2 as $value3) {
 ++$pRow1;
 ++$pRow2;
 $sheet->fromArray($value3, NULL, "A$pRow1");
 }
 }
 }
}
