	...
	if ($action == 'A') {
		...
		$rows2 = $daomapa->buscaMapaObjetivoPerspectivaPorDocumento($objetivo->getCodigo(), $perspectiva->getCodigo(), $documento->getCodigo());
 		if(!($rows2->rowCount() == 0)){
			...
			$rows= $daomapa->buscaMapaObjetivoPerspectivaPorDocumento($objmapa->getObjetivoPDI(), $objmapa->getPerspectiva(), $objmapa->getDocumento());
			foreach ($rows as $r){
				$codmapapdu=$r['Codigo'];
			}
			
	        $rowspdi=$daomapaind->buscaIndicadorVinculadoAoObjetivoIncluidoNoPDU($sessao->getAnobase(), $objetivo->getCodigo(), $propMapa);
		    foreach ($rowspdi as $r){
				 $rowsmi=$daomapaind->buscamapa($r['codIndicador'], $codmapapdu, $propMapa);//vê se o indicador já está no mapa do pdu
				 $codigo=0;
				 foreach ($rowsmi as $o){
				 ...
				 if ($codigo==0){//o indicador do pdi nao está no pdu
				   $daomapaind->insere($r['codIndicador'], $codmapapdu, $propMapa);
				   $passou=1;
				   $rows=$daomapaind->buscamapa($r['codIndicador'], $codmapapdu, $propMapa);
				   foreach ($rows as $r1){
