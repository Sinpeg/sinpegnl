...
/* Validação */
 if ($codunidade==938 && ($perspectiva->getCodigo() == "" || $perspectiva->getCodigo() == 0)){
    ...
    //pegar ano inicial do pdu
	$rowspdu=$daodoc->buscadocumento($documento->getCodigo())->fetch();
	$objmapa->setAnoinicio($rowspdu['anoinicial']);	
	$objmapa->setPeriodoinicial("2");
...
// 		}
		...
		$rows2 = $daomapa->buscaMapaObjetivoPerspectivaPorDocumento($objetivo->getCodigo(), $perspectiva->getCodigo(), $documento->getCodigo(),$sessao->getAnobase());
 		
 		//echo $objetivo->getCodigo()."/".$perspectiva->getCodigo()."/".$documento->getCodigo()."/".$sessao->getAnobase();
