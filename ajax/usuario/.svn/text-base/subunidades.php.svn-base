...
if (!empty($subunidades)){
   ...
   $display=  "<select id='select1' name='lista1[]' multiple size=multiple style='width: 600px' >";
   foreach ($subunidades as $res) {
	$display.=  "<option value=".$res['CodUnidade'].">".$res['NomeUnidade']."</option>";
   }
   $display.= "</select>";
}else{
  $display="<select id='select1' name='lista1[]' multiple size=multiple style='width: 600px' ></select>";
}
