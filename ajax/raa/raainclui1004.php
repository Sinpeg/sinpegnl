...
if ($anobase<=2020){
    print '<div style="width:100%;font-size:10px" class="table-wrapper-scroll-y my-custom-scrollbar">
    ...
    <table disabled="disabled" class="table table-bordered table-striped mb-0" style="width:100%">
	    <tr bgcolor="#CCCCCC" >
            ...
            <td colspan=5 style="text-align:center"><b>Crédito de Custeio</b></td>
    ...
    if ($anobase<2022){
        print '<div style="width:300px;font-size:10px"><table disabled="disabled" style="width:100%">
	    <tr bgcolor="#CCCCCC" >
    ...
    }else{//Ano base maior ou igual a 2022
	    print '<div style="width:100%;font-size:10px"><table disabled="disabled" style="width:100%">
	    <tr bgcolor="#CCCCCC" >
		    <td rowspan=3 style="text-align:center;vertical-align: middle;width=10"><b>PI</b></td>
            <td rowspan=3 style="text-align:center;vertical-align: middle"><b>Projeto</b></td>
            <td colspan=7 style="text-align:center"><b>Crédito de Custeio</b></td></tr>';
...
foreach ($rowsOrca As $rows){
    if ($anobase<=2020){
        $somatorio=$rows['custeio_previsto']+$rows['custeio_reprogramado']+$rows['custeio_apoio']
        +$rows['custeio_recebido_proad']+$rows['custeio_disponibilizado']+$rows['custeio_movimentada']+
        $rows['custeio_empenhada']+$rows['custeio_liquidada'];
    }else{
        if ($anobase<2022){
            $somatorio=$rows['custeio_previsto']+$rows['custeio_reprogramado']+$rows['custeio_apoio']
            +$rows['custeio_liberado']+$rows['custeio_disponibilizado']+$rows['custeio_movimentada']+
            $rows['custeio_empenhada']+$rows['custeio_liquidada'];
        }else {
	        $somatorio=$rows['custeio_previsto']+$rows['custeio_reprogramado']+$rows['custeio_apoio']
            +$rows['custeio_disponibilizado']+$rows['custeio_movimentada']+
            $rows['custeio_empenhada'];
        }
    ...
    if ($somatorio>0){
        ...
        if ($anobase<=2020){
            ...
            print '<tr><td>'.$rows['pi'].'</td><td>'.$rows['projeto'].'</td><td>R$'.
                ...
                $totalca+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_recebido_proad']));
            ...
            if ($anobase<2022){
                ...
                $total=$custeio_movimentado+$custeio_empenhado;
                ...
                print '<tr><td>'.$rows['pi'].'</td><td>'.$rows['projeto'].'</td><td>R$'.
                    ...
                    $rows['custeio_liquidada'].'</td></tr>';
                    $totalca+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_liberado']));
            }else{//Quando o ano base é maior ou igual a 2022
                $rows['custeio_liberado'] = ($rows['custeio_liberado']== null || $rows['custeio_liberado']== "") ? "0,00" : $rows['custeio_liberado'] ;
                $total=0;
                $custeio_movimentado=($rows['custeio_movimentada']== null || $rows['custeio_movimentada']== "") ? "0" : $rows['custeio_movimentada'] ;
                $custeio_empenhado=($rows['custeio_empenhada']== null || $rows['custeio_empenhada']== "") ? "0" : $rows['custeio_empenhada'] ;
                ...
                print '<tr><td>'.$rows['pi'].'</td><td>'.$rows['projeto'].'</td><td>R$'.
                    number_format($rows['custeio_previsto'],2,",",".").'</td><td>R$'.
                    number_format($rows['custeio_reprogramado'],2,",",".").'</td><td>R$'.                    
                    number_format($rows['custeio_apoio'],2,",",".").'</td><td>R$'.
                    number_format($rows['custeio_disponibilizado'],2,",",".").'</td><td>R$'.
                    number_format($rows['custeio_movimentada'],2,",",".").'</td><td>R$'.
                    number_format($rows['custeio_empenhada'],2,",",".").'</td><td>R$'.
    ...
    $totalcp+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_previsto']));
    $totalcr+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_reprogramado']));
    $totalcl+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_apoio']));
    $totalca+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_recebido_proad']));
    $totalcd+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_disponibilizado']));
    $totalcm+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_movimentada']));
    $totalce+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_empenhada']));
    $totalcq+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['custeio_liquidada']));
    $tem++;
}

if ($anobase<=2020){
    
    print '<tr><td colspan=2>Totais</td><td>R$'.
     ...
     if ($anobase<2022){
        ...
        number_format($totalcm,2,",",".").'</td><td>R$'.
        number_format($totalce,2,",",".").'</td><td>R$'.
        number_format($total_total1,2,",",".").'</td></tr>';       
        //'<td>R$'.        
        //number_format($totalcq,2,",",".").'</td></tr>';
     }
}

print'</table>Fonte:PGO-'.$anobase.' e SIAFI 30/12/'.$anobase.'<br></div></br>';
...
print 'Quadro 3 - Desempenho do Orçamento de Capital no Exercício por Plano Interno
<div style="width:100%;font-size:10px"><table disabled="disabled" style="width:100%">';
...
if ($anobase<=2020){
    ...
    if ($anobase<2022){
        ...
        print '<tr bgcolor="#CCCCCC" >
            <td rowspan=3 style="text-align:center;vertical-align: middle"><b>PI</b></td>
            <td rowspan=3 style="text-align:center;vertical-align: middle"><b>Projeto</b></td>
            <td colspan=7 style="text-align:center"><b>Crédito de Capital</b></td></tr>';
...
foreach ($rowsOrca As $rows){
    $totalcp+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_previsto']));
    $totalcr+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_reprogramado']));
    $totalcl+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_apoio']));
    if ($anobase<=2020){
        $totalca+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_recebido_proad']));
    }else{
        $totalca+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_liberado']));
    }
    $totalcd+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_disponibilizado']));
    $totalcm+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_movimentada']));
    $totalce+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_empenhada']));
    $totalcq+=str_replace(",",".",preg_replace("/[^0-9,]+/i","",$rows['capital_liquidada']));
    
    
    
    if ($anobase<=2020){
        $somatorio=$rows['capital_previsto']+$rows['capital_reprogramado']+$rows['capital_apoio']+$rows['capital_recebido_proad']
        +$rows['capital_disponibilizado']+$rows['capital_movimentada']+$rows['capital_empenhada']+$rows['capital_liquidada'];
        
    }else{
        if ($anobase<2022){
            $somatorio=$rows['capital_previsto']+$rows['capital_reprogramado']+$rows['capital_apoio']+$rows['capital_liberado']
            +$rows['capital_disponibilizado']+$rows['capital_movimentada']+$rows['capital_empenhada']+$rows['capital_liquidada'];
        }else{
            $somatorio=$rows['capital_previsto']+$rows['capital_reprogramado']+$rows['capital_apoio']+$rows['capital_disponibilizado']+$rows['capital_movimentada']+$rows['capital_empenhada'];
        }
    }
    ...
    if ($somatorio>0){
        ...
        if ($anobase<=2020){
            ...
            if ($anobase<2022){
                ...
                $total=$capital_movimentado+$capital_empenhado;
                ...
                print '<tr><td>'.$rows['pi'].'</td><td>'.$rows['projeto'].'</td><td>R$'.
                    number_format($rows['capital_previsto'],2,",",".").'</td><td>R$'.
                    number_format($rows['capital_reprogramado'],2,",",".").'</td><td>R$'.
                    //$rows['capital_liberado'].'</td><td>R$'.
                
                    number_format($rows['capital_apoio'],2,",",".").'</td><td>R$'.
                    number_format($rows['capital_disponibilizado'],2,",",".") .'</td><td>R$'.
                    number_format($rows['capital_movimentada'],2,",",".").'</td><td>R$'.
                    number_format($rows['capital_empenhada'],2,",",".").'</td><td>R$'.                
                    $total.'</td></tr>';                
...
if ($anobase<=2020){
    ...
    if ($anobase<2022){
        ...
        number_format($totalcm,2,",",".").'</td><td>R$'.
        number_format($totalce,2,",",".").'</td><td>R$'.
        number_format($total_total2,2,",",".").'</td></tr>';
        //number_format($totalcq,2,",",".").'</td></tr>';
    }
}
...
/*
 ...
 print 'Percentual recebido do Crédito de Custeio ('.$percentRecebidoDoRecCusteio.'%) x
 Percentual recebido do recurso de capital ('.$percentRecebidoDoRecCapital.'%)<br>';
 print 'Percentual de custeio utilizado '. $percentCusteioUtilizado. "% x " ."Percentual de capital utilizado ". $percentCapitalUtilizado."%<br>";
 print 'Percentual da despesa de custeio liquidada '.$percentDespesaCusteioLiquidada. "% x " ."Percentual da despesa de capital liquidada ". $percentDespesaCapitalLiquidada."%<br>";
 */
