<?php
require '../../classes/sessao.php';
require_once '../../dao/PDOConnectionFactory.php';
require_once '../../dao/unidadeDAO.php';
require_once '../../modulo/raa/dao/modeloDAO.php';
require_once '../../modulo/raa/dao/textoDAO.php';
require_once '../../modulo/raa/dao/topicoDAO.php';
require_once '../../modulo/raa/classes/topico.php';
require_once '../../modulo/raa/classes/texto.php';
require_once '../../modulo/raa/classes/modelo.php';
require_once '../../classes/unidade.php';
require_once '../../modulo/documentopdi/dao/DocumentoDAO.php';
require_once '../../modulo/raa/funcoesraa.php';
require_once '../../modulo/calendarioPdi/dao/CalendarioDAO.php';//carla - para bloquear a aplicação no prazo

session_start();
$sessao = new Sessao();
$sessao = $_SESSION['sessao'];
$anobase = $sessao->getAnobase();
$codunidade = $sessao->getCodUnidade();
$nomeunidade = $sessao->getNomeunidade();
$codtexto = $_POST['codtexto'];
$codTopico = $_POST['codTopico'];

$lista = array();
$cont = 0;

$mdao= new ModeloDAO();

$t=new Topico();
$t->setCodigo($codTopico);

$tdao= new TextoDAO();


$msg="";
//Verificar se o relatório foi finalizado
$disabled = "";
$frows = $tdao->buscaFinalizacaoRel($codunidade, $anobase);
foreach ($frows as $row){
	$disabled = "disabled";
}
//carla - para bloquear a aplicação no prazo
$daocal=new CalendarioDAO();
$rows=$daocal->verificaPrazoCalendarioDoRAA($anobase);
foreach ($rows as $row) {
	//echo $row['habilita'];
     $disabled=$row['habilita']=='D'?"disabled":$disabled;
}

$trows=$tdao->buscaTexto($codTopico, $anobase,$codunidade);

/*foreach ($trows as $r){

	$uniTexto=new Unidade();
	$uniTexto->setCodunidade($r['codUnidade']);
	$t->criaTexto($row['codtexto'],$row['texto'],$uniTexto,$row['ano']);

}*/

?>
<script>
	$("#gravatopico<?php print $t->getCodigo()?>").click(function() {
			$.ajax({
				url:"ajax/raa/registraTexto.php",
				type: "POST",
				data: $("form[name=frm<?php print $t->getCodigo();?>]").serialize(),
				success: function(data) {
					$("div#msg<?php print $t->getCodigo()?>").html(data);
				}

			});
		});
		$(document).ready(function() {
			var teste="#area<?php print $t->getCodigo()?>";
				$(teste).summernote({
					height: 450,
					lang:'pt-BR',
					placeholder: 'Redija o texto!',
					fontSize:'12px'
				});
				if(<?php print $t->getCodigo()?>==2 || <?php print $t->getCodigo()?>==3 || <?php print $t->getCodigo()?>==5){
					$(teste).summernote('disable');
				}
		});
</script>

<?php //Verificar habilitação do tópico "Gestão Orçamentária e Financeira"
// alterado porque estava apresentando a mensagem do ano atual em anos anteriores
$rowsOrca = $mdao->buscarorcamentocusteio($codunidade, $anobase);
$passou=0;
foreach ($rowsOrca as $r){
    $passou=1;
}
// Liberar para as Unidades 961(arqivo central), 943(ouvidoria), 960(gráfica), 965(CMA), 7111(ADIS) - não têm orçamento
if((date("d-m-Y") < strtotime('14-01-2023') && $t->getCodigo()==145 && $passou==0) && ($codunidade!=961 && $codunidade!=943 && $codunidade!=960 && $codunidade!=965 && $codunidade!=7111)){
//if((date("d-m-Y") < strtotime('14-01-2022') && $t->getCodigo()==145) ){ //até aqui
print '<div class="alert alert-warning" role="alert">As informações estarão disponíveis a partir do dia 14/01/2023, devido ao fechamento do SIAFI.</div>';

}else{?>

<form method="post" id="redigirRaa" name="frm<?php print $t->getCodigo();?>" >
               <input type="hidden" name="codunidade>" value="<?php print $codunidade;?>" />
               <input type="hidden" name="anobase" value="<?php print $anobase;?>" />
		       <input type="hidden" name="codtopico" value="<?php print $codTopico?>" />



<?php
$codtexto='';

	$rowsa=$mdao->buscarmodelosUniTopAno1($codTopico,$anobase,$codunidade);

	if ( ($t->getCodigo()==143 || $t->getCodigo()==145) AND $anobase>=2019){//tópicos orçamento e planejamento(145)Resultados(143)
     
	  include "inclusoesraa.php";
	
	}else if($t->getCodigo()==2 ){//and $anobase>=2019){ Tópico da capa
		//Gerar capa?>
		<textarea id="area<?php print $codTopico;?>" name="texto"  <?php print $disabled;?>>

	<?php 	print '<br><br><p style="text-align: center; "><img src="data:image/png;base64," data-filename="ufpa.png" style="width: 87px; height: 102.225px;">
		<br></p><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""="">UNIVERSIDADE
FEDERAL DO PARÁ</span></p><p class="MsoNormal" style="text-align:center" align="center"><b><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;'.$nomeunidade.'&nbsp;</o:p></span></b></p><p class="MsoNormal" style="text-align:center" align="center"><b><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p><br></o:p></span></b></p><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;</o:p></span></p><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;</o:p></span></p><br><br><br><p class="MsoNormal" style="text-align:center" align="center"><b><span style="font-size:18.0pt;line-height:115%;font-family:" arial","sans-serif""="">RELATÓRIO
ANUAL DE ATIVIDADES&nbsp;<o:p></o:p></span></b></p><p class="MsoNormal" style="text-align:center" align="center"><b><span style="font-size:18.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;</o:p></span></b></p><p class="MsoNormal" style="text-align:center" align="center"><b><span arial","sans-serif""="" style="font-size: 18pt; line-height: 27.6px;">EXERCÍCIO -&nbsp;</span></b><font face="Arial, sans-serif"><span style="font-size: 24px;"><b>'.$anobase.'</b></span></font></p><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp; <br></o:p></span></p><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;</o:p></span></p><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;</o:p></span></p><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;</o:p></span></p><p class="MsoNormal" style="text-align:center" align="center">
		<span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""=""><o:p>&nbsp;</o:p></span></p>
		<br><br><br><br><br><br><br><br><p class="MsoNormal" style="text-align:center" align="center"><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif""="">'.$anobase.'<o:p></o:p></span></p><p style="text-align: center; "><span style="font-size:12.0pt;line-height:115%;font-family:" arial","sans-serif";="" mso-fareast-font-family:calibri;mso-ansi-language:pt-br;mso-fareast-language:="" en-us;mso-bidi-language:ar-sa"=""></span><br></p><p><br></p><p><br></p>';
	?>
		</textarea>

<?php
	}elseif($t->getCodigo()==5){ //Gerar Sumário?>
		<textarea id="area<?php print $codTopico;?>" name="texto"  <?php print $disabled;?>>
		<?php
			//Buscar sumário
			$rowsl = $tdao->buscaTopicosPreenchidos_Pendentes($anobase, $codunidade);
			$topico=array();
			$topico=incluirTopicoNoTexto($rowsl,$topico,$cont,$codunidade,$anobase);
			$rowsl = $tdao->buscaSubTopicosPreenchidos_Pendentes($anobase, $codunidade);
			$topico=incluirTopicoNoTexto($rowsl,$topico,$cont,$codunidade,$anobase);
			$aux=0;$nivel=0;

			print "Relação dos Dirigentes da Administração Superior<br/>";

			if (count($topico)>0)  {
				$anterior=NULL;
				foreach ($topico as $tt){
					$trows = $tdao->buscaTexto($tt->getCodigo(), $anobase, $codunidade);
					$codtexto='';
					$passou=0;

					if($trows->rowCount()>0){

						if($tt->getNivel() != NULL){
							$nivel++;
							print $nivel.". ".$tt->getNome()."<br/>";
						}else{
							print "".$tt->getNome()."<br/>";
						}
					}
					/*
					$aux =1;
					foreach ($trows as $r){
						$html .= $r['texto'];
						$codtexto=$r['codigo'];
						$passou=1;

					}
					*/
					if ($tt->getSubtopicos()!=NULL ){
						 subtopicoS($tt->getSubtopicos(),$codunidade,$anobase,$nivel);
					}else if($passou==0 AND $tt->getCodigo()!=2){
						//$html .= "Não se aplica!";

					}
				}
			}
		?>
		</textarea>
<?php
	}else{//outros tópicos?>
	<textarea id="area<?php print $codTopico;?>" name="texto"  <?php print $disabled;?>>
	<?php
	 $passou=0;

	foreach ($trows as $r){
		print $r['texto'];
		$codtexto=$r['codigo'];
	    $passou=1;

	 }
	 if ($passou==0){//Quando não possui registro para o ano base
	    //Buscar dados do ano anterior
	    $anobaseAnterior = $anobase-1;
	    $trows2=$tdao->buscaTexto($codTopico, $anobaseAnterior,$codunidade);
	    //121 Relação dos Dirigentes da Unidade 
	    //140 Competências das subunidades e setores da unidade
	    //141 Dirigentes da Unidade
	    //146 Previsão acumulada
	    if($trows2->rowCount()>0 AND ($codTopico == 121 || $codTopico == 140 || $codTopico == 141)){
	        foreach ($trows2 as $r1){
	            print $r1['texto'];
	            $msg = '<div class="alert alert-danger" role="alert">O conteúdo apresentado deste Tópico foi importado do relatório anterior. Atualize os dados e clique no botão "Gravar". Caso não haja necessidade de atualiza-los, apenas clique no botão "Gravar" </div>';
	        }
	    }else{
	        if ( ($codTopico ==211 || $codTopico ==212) && $anobase>2021){//buscar modelo e dados para preencher a tabela de previsão acumulada
	            $rowsac=$mdao->buscarPrevAcumulada($codunidade,$anobase);
	            print tabelaPrevisaoAcumulada($rowsac);
	            
	        }
	        else{ //Buscar modelo(s) do item
	        
    	        //Buscar modelo(s) do item
    	        foreach ($rowsa as $r){
    	            print($r['legenda'].$r['descModelo']);
    	        }
	        }
	    }
	    
	    
	 }
	?>

	</textarea>
<?php } ?>
       <input type="hidden" name="codtexto" value="<?php print $codtexto?>" />

<br>
<div id="msg<?php print $t->getCodigo()?>"></div>
<br/>
<?php
print $msg;
$flagNaoResult = 0; //Adicioonado em decorrência de um warning

if($t->getCodigo()!=2 && $t->getCodigo()!=3 && $t->getCodigo()!=5 && $flagNaoResult==0){?>
<center><input type="button" name="acao"<?php print $disabled;?> value="Gravar" id="gravatopico<?php print $t->getCodigo();?>"  class="btn btn-primary btn"></center>
<?php 

}?>
</form>

<?php } //da desabilitacao da gestao orcam?>

<style type="text/css">
.classediv{
           font-size:11px;

}

.tabelaexpansao {
      border:1px solid transparent; 
      border-collapse:collapse;
      
 } 

    
 .cabexpansaoserv1{
     cellspacing:50;
     background:#dddddd;
     text-align:center
     
   }
 
  
   .tabexpansaoserv1{
     cellspacing:50;
     text-align:center;
     
 }
  
   
   
   .tabexpansaoserv2{
     background:#dddddd;
     cellspacing:50;
     text-align:center;
     
     
 }


</style>
<?php 
function tabelaPrevisaoAcumulada($rowsac){
    
    
    
    
    $tabela= '<p><b>Tabela 2 - Quantidade de servidores previstos no PDU para o exercício e os em atividade</b></p><br>';
    
    
    
    foreach ($rowsac as $r1){
        $paMs= is_null( $r1['prevAcumMS'] )?0:$r1['prevAcumMS'];
        $paEBTT=is_null( $r1['prevAcumEBTT'])?0:$r1['prevAcumEBTT'];
        $paA=is_null($r1['prevAcumA'])?0:$r1['prevAcumA'];
        $paB=is_null($r1['prevAcumB'])?0:$r1['prevAcumB'];
        $paC=is_null($r1['prevAcumC'])?0:$r1['prevAcumC'];
        $paD=is_null($r1['prevAcumD'])?0:$r1['prevAcumD'];
        $paE=is_null($r1['prevAcumE'])?0:$r1['prevAcumE'];
        
    }
    
    $total= $paMs+$paEBTT+$paA+$paB+$paC+$paD+$paE;
    
    $tabela.='<div class="classediv"><table class="tabelaexpansao">
    <tr>
    <th class="cabexpansaoserv1">Categorias</th>
    <th class="cabexpansaoserv1">Cargo/Tipologias dos Cargos</th>
    <th class="cabexpansaoserv1">Previsão Acumulada (1)</th>
    <th class="cabexpansaoserv1">Em Atividade no Exercício (2)</th>
     </tr>
    
    <tr>
    <td rowspan=2 class="cabexpansaoserv1">Docente</td>
    <td class="cabexpansaoserv1">Professor de Magistério Superior</td>
    <td class="tabexpansaoserv1">'.$paMs.'</td>
    <td class="tabexpansaoserv1"></td>
    </tr>
	    
    <tr>
    <td class="cabexpansaoserv1">Professor EBTT</td>
    <td class="tabexpansaoserv1">'.$paEBTT.'</td>
    <td class="tabexpansaoserv1"></td></tr>
	    
    <tr  class="trexpansao">
       <td rowspan=5 class="cabexpansaoserv1">Técnico-Administrativo</td>
       <td class="cabexpansaoserv1">1 – Classe A</td>
       <td class="tabexpansaoserv1">'.$paA.'</td>
        <td class="tabexpansaoserv1"></td></tr>
	    
    <tr>
      <td class="cabexpansaoserv1">2 – Classe B</td>
      <td  class="tabexpansaoserv1">'.$paB.'</td>
      <td  class="tabexpansaoserv1"></td></tr>
								       
								       
    <tr>
     <td class="cabexpansaoserv1">3 – Classe C</td>
     <td  class="tabexpansaoserv1">'.$paC.'</td>
     <td  class="tabexpansaoserv1"></td></tr>
								          
    <tr>
      <td class="cabexpansaoserv1">4 – Classe D</td>
      <td  class="tabexpansaoserv1">'.$paD.'</td>
      <td  class="tabexpansaoserv1"></td></tr>
									    
									    
    <tr>
        <td class="cabexpansaoserv1">5 – Classe E</td>
        <td  class="tabexpansaoserv1">'.$paE.'</td>
        <td class="tabexpansaoserv1"></td></tr>
									          
									          
    <tr>
      <td class="cabexpansaoserv1"  colspan=2>Total de Servidores</td>
      <td class="tabexpansaoserv2">'.$total.'</td>
    <td class="tabexpansaoserv2"></td>
</tr>
    </table></div>';
    
    $tabela.='<p class="Epgrafe"><b><span style="font-size: 9px; font-family: Helvetica;">Fonte:&nbsp;</span></b><br class="Apple-interchange-newline"></p><p class="Epgrafe"><span style="font-size: 9px; font-family: Helvetica;">1 - Previsão Acumulada = Soma da quantidade de servidores efetivos da UFPA informado no PDU mais a meta prevista de expansão acumulada até o exercício de referência do relatório de atividades.</span><o:p></o:p></p><p class="MsoCaption" style="margin-right: -0.05pt; text-align: justify;"><span style="font-size: 9px;"></span><span times="" new="" roman",serif;mso-fareast-font-family:="" "times="" roman";mso-ansi-language:pt-br;mso-fareast-language:pt-br;="" mso-bidi-language:ar-sa"="" style="font-size: 9px; font-family: Helvetica;">2 - Em Atividade no Exercício = Total de servidores efetivos da UFPA em 31/12, excluindo cedidos ou removidos ou redistribuídos.</span></p>';
    
    
    
    return $tabela;
}




?>
