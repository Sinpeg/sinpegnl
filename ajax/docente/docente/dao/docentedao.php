...
`autor`, `texto`) VALUES (?,?,?,?)";
    ...
    public function buscaDocente32($iddocente) {
        try {
            $sql = "SELECT c.idcursocenso, c.nome,c.idcursoinep,iddocente FROM `docente32` d join cursocenso c on c.idcursocenso=d.idCursoinep
              WHERE iddocente=:cod";
            $stmt = parent::prepare($sql);
    ...
    public function buscaCursos($codtempo) {
        try {
            $sql = "SELECT * FROM cursosatuacao WHERE  codtempo=:codtempo";
            $stmt = parent::prepare($sql);
