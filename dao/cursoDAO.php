...
<?php
        ...
        public function Listacomparacao() {//censo
            try {
                
                $stmt = parent::query("SELECT * FROM comparacao2020");
                
                // retorna o resultado da query
                return $stmt;
            } catch (PDOException $ex) {
                echo "Erro Listacomparacao: " . $ex->getMessage();
            }
        }
    ...
    public function Listacenso() {
        try {
            
            $stmt = parent::query("SELECT `CodCurso`,  `CodCampus`, `CodUnidade`,
`CodNivel`, `CodCursoSis`, `NomeCurso`, `Sigla`,  `CodEmec`, `Regime`, c.`Modalidade`, c.`Formato`,
`TipoCurso`,  `CodSigaa`, `CodIntegracao`,
  c.`Situacao`, c.`AnoValidade`,s.idcursoinep
FROM curso c join cursocenso s on c.codemec=s.idcursocenso");
            
            // retorna o resultado da query
            return $stmt;
        } catch (PDOException $ex) {
            echo "Erro Listacenso cursodao: " . $ex->getMessage();
        }
    }
...
FROM curso c  left join cursocenso cc on c.codemec=cc.idcursocenso where ";
            if ($anobase<=2013){
            ...
            }else if($anobase >= 2018 && $anobase<2021){
                ...
                $sql .= "  `AnoValidade` = 2021 and codnivel=1 ";
                $sql .= " and c.CodCurso not in (select c1.CodCurso
                    from laboratorio_curso c1
                    where c1.Codcurso=c.Codcurso and ano=2021 and codlaboratorio=:codlab)";
            }
    ...
    public function buscaCursosUnidade($CodUnidade, $anobase) {
        try {
            $sql = "SELECT cur.`CodCurso` ,cur.`CodUnidade` ," .
                " cur.`CodNivel` , cur.`CodCursoSis` , cur.`NomeCurso` , cc.`idcursoinep` as CodEmec , cur.`DataInicio` " .
                " FROM  `curso` cur
                     left outer join `cursocenso` cc on cc.`idCursocenso` = cur.`CodEmec`  AND cc.Situacao=1  ".
                     " WHERE cur.CodUnidade =:codunidade " .
            ...
            if ($anobase<=2013){
                ...
                $sql .= "AND cur.`AnoValidade` = 2014";
            }else if  (2017 <= $anobase && $anobase <= 2018 ){
                $sql .= "AND cur.`AnoValidade` = 2018";
            }else if($anobase >= 2021){
                $sql .= "AND cur.`AnoValidade` = 2021";
            }
            //$sql .=" order by cc.`idcursoinep`";
    ...
    public function buscaCursosUnidadeTec($CodUnidade, $anobase) {
        try {
            $sql = "SELECT cur.`CodCurso` , cam.`CodCampus` codcampus, cam.`Campus` nomecampus, cur.`CodUnidade` ," .
                " cur.`CodNivel` , cur.`CodCursoSis` , cur.`NomeCurso` , cc.`idcursoinep` as CodEmec , cur.`DataInicio` " .
                " FROM `campus` cam ".
