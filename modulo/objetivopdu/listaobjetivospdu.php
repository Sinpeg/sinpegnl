...
for ($i = 0; $i < $cont; $i++) {
    ...
    foreach ($rows as $row) {
        if (isset($row['codObjetivoPDI'])) {
            ...
            $objmapa[$cont1]->setCodObjetivoPDI($row['codObjetivoPDI']);
            $cont1++;
        }
    }
}
if ($cont1 == 0) {
    Utils::redirect('objetivopdu', 'incluiobjetivospdu');
    exit;
}
?>
<script>
...
<h3 class="card-title">Objetivos Estratégicos - PDU</h3>
    ...
    <tbody>
        <?php for ($i = 0; $i < $cont1; $i++): ?>
            <tr>
                <?php
                $daomapa = new MapaDAO();
                $rows = $daomapa->buscamapa($objmapa[$i]->getCodObjetivoPDI());
                $objmapa1 = new Mapa();
                foreach ($rows as $row) {
                    $objmapa1->setObjetivo($row['Objetivo']);
                }
                ?>
                <td><?php print $objmapa1->getObjetivo(); ?></td>
                <td><?php print ($objmapa[$i]->getOrdem()); ?></td>
                <td><?php print ($objmapa[$i]->getObjetivo()); ?></td>
                <td><?php print ($objmapa[$i]->getDocumento()->getNome()); ?></td>
                <td><a href="<?php echo Utils::createLink('objetivopdu', 'editarobjetivopdu', array('codmapa'=>$objmapa[$i]->getCodigo())); ?>" title="Editar o valor da meta"><img src="webroot/img/editar.gif"/></a></td>
            </tr>
        <?php endfor; ?>
    </tbody>
</table>
<div class="incluir">
    <span class="plus"></span><a href="<?php echo Utils::createLink('objetivopdu', 'incluiobjetivospdu'); ?>">Incluir Objetivo Estratégico - PDU</a>
</div>
