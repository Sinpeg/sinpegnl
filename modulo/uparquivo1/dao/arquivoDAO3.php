...
*/
	...
	public function buscaCodigo($codigo){
		try{
			...
			$stmt = parent::prepare("SELECT `Nome`,`Tipo`,`Conteudo` , `Tamanho` FROM `arquivo` WHERE  `Codigo`=:codigo");
	...
	public function buscaporCodigo($codigo){
		try{
			$stmt = parent::prepare("SELECT `Nome`,`Tipo`,`Assunto`,`Comentario` ,`Tamanho`,`DataInclusao`,`DataAlteracao` FROM `arquivo` WHERE  `Codigo`=:codigo");
			$stmt->execute(array(':codigo'=>$codigo));
			return $stmt;
		}catch ( PDOException $ex ){
			$mensagem = urlencode($ex->getMessage());//para aceitar caracteres especiais
