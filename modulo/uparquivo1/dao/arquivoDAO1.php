...
class ArquivoDAO extends PDOConnectionFactory {
    ...
    public function buscaUniUsuario($anobase, $codunidade, $codusuario) {
   	...
   	public function buscaCodigo($codigo){
   		try{
            $sql="SELECT `Nome`,`Tipo`,`Conteudo` ,`Tamanho` FROM `arquivo` WHERE  `Codigo`=:codigo";

   			$stmt = $this->conex->prepare($sql);


   			$stmt->execute(array(':codigo' => $codigo)); //or die(implode(":",$stmt->errorInfo()));;
   		    $row=$stmt->fetch();
echo $row['Tipo'];
    ...
    echo $row['Conteudo'] ;
   			...
   			die("Conexao falhou".$ex->getMessage());
   		}
   	}
