        ...
        foreach ($rows as $row) 
        {
            ...
            foreach ($tiposlab as $tipolab) 
            {
                if ($tipolab->getCodigo() == $tipo) 
                {
                    ...
                    if (!$sessao->isUnidade() && Utils::isApproved(7, $codunidadesup, $codunidade, $lab->getAnoativacao()) && $anobase == $lab->getAnoativacao()) {
                        $button = false;
                    }
...
<div class="bs-example">
    ...
    <div class="ui-state-highlight ui-corner-all" style="padding: 0 .7em;">
        ...
        <p>Cadastre <strong>TODOS</strong> os laboratórios da sua unidade, porque a área registrada é utilizada no cálculo da Matriz Orçamentária.</p>
...
<?php if ($cont > 0) { ?>
    ...
    for ($i = 0; $i < count($locks); $i++) {
        ...
        <?php else: ?>
                    ...
                    <?php endif; ?>
                    <?php if (!$locks[$i]->getLocked()): ?>
                    <?php $num = $daolab->buscaVinculoLabCurso($lab1->getCodlaboratorio(),$sessao->getAnobase());
                    	
                    ?>
                        <td align="center">
                        <?php foreach ($num as $row){
                        	if($row['qtdCursos'] >= 1){
                        		 ?> <a href="<?php echo Utils::createLink('labor', 'conslabcurso', array('codlab' => $lab1->getCodlaboratorio())); ?>" target="_self"><img src="webroot/img/busca.png"  alt="Visualizar"/> </a>
                        	<?php }else{?>
                        	    
                        		<a href="#" onclick="direcionacurso(1,<?php echo $lab1->getCodlaboratorio()?>);" target="_self"> <img src="webroot/img/add.png"  alt="Adicionar"/> </a>
                        	<?php }
                        }?>
