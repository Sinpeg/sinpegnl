...
`AnoDesativacao`=? WHERE CodLaboratorio=?");
    ...
    public function alteraSit($laboratorio) {
        try {
            $stmt = parent::prepare("UPDATE `laboratorio` SET`Situacao`=?
 WHERE CodLaboratorio=?");
            parent::beginTransaction();
           
            $stmt->bindValue(1, $laboratorio->getSituacao());
            $stmt->bindValue(2, $laboratorio->getCodlaboratorio());
            $stmt->execute();
            parent::commit();
        } catch (PDOException $ex) {
            parent::rollback();
    ...
    public function buscaLaboratorio($codlaboratorio, $ano) {
        ...
        if ($ano<=2019)
            $rsql=" and validade=2019 ";
         else $rsql=" and validade=2021 ";
...
l. `Situacao`, `AnoAtivacao`, `AnoDesativacao`,

lc. `Codtipo`,lc. `ano`, `sugestaoDeTipo`, `inforAdicional`

 
             FROM laboratorio l 
             left join lab_classecenso lc on l.codlaboratorio=lc.codlaboratorio and lc.ano=:ano
             left join tdm_tipo_laboratorio t on t.codigo=lc.codtipo" .$rsql.
    ...
    public function listaClasseCenso($codlaboratorio,$ano) {
        try {
            if ($ano<=2019)
                $rsql=" and validade=2019 and lc.ano<2021";
                else $rsql=" and validade=2021 and lc.ano=2021 ";
          $sql= " SELECT t.codigo
            from lab_classecenso lc
            join tdm_tipo_laboratorio t on t.codigo=lc.codtipo
            where lc.Codlaboratorio=:codlaboratorio ".$rsql; 
...
join laboratorio l on t.Codigo = l.Tipo
    ...
    public function buscaLaboratoriosUnidade($CodUnidade,$ano) {
        try {
            ...
            and t.Codigo = l.Tipo and anodesativacao is null or anodesativacao>:ano");
