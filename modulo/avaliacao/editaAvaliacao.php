...
//$arrayAvalfin = $daoAvaliacaofinal->buscaArquivoAvalCodDoc($documento['codigo'], $sessao->getAnobase());  //possui o erro Call to unknown method: AvaliacaofinalDAO::buscaArquivoAvalCodDoc()PHP(PHP0418)
$arrayAvalfin = $daoAvaliacaofinal->buscaArquivoAval($documento['codigo']);

//var_dump($arrayAvalfin);
...
<div class="card card-info">
	<div class="card-header"><h3 class="card-title">Registrar / Editar Avaliação</h3></div>
	<form class="form-horizontal" enctype="multipart/form-data" name="adicionar" id="adicionar" method="POST" >
		...
		<div id="tabela">	
			<table>
				...
				<tr>                     
					<td>
						<label>Ata Avaliacao</label>
					</td>
					<td>
						<input class="form-control" class="custom-file-input" type="file"  name="userfile" id="userfile" />
				...
				<?php if($edicao == 1){?>
				<tr> 
						<td><input class="form-control"type ="hidden" name ="userlink" value="true"/></td>                    
				...
				<?php }else{ ?>
					<tr>
						<td>
							<input class="form-control"type ="hidden" name ="userlink" value="false"/>
						</td>
					</tr>
				<?php }?>
				<tr>
					<td align="center" colspan="2">
						<input class="form-control"type="hidden" value="<?php if($edicao == 1){print "editar";}else{print "gravar";}?>" name="funcao" />
						<input type="button" id="gravar" value="<?php if($edicao == 1){ print "Atualizar"; }else{ print "Gravar";}?>" onclick="direciona(1);" name="salvarr" class="btn btn-info"/>
					</td>
				</tr>
			</table>
		</div>
