...
//echo "altx ".$codlab.",".$ano.",".$area.",".$justificativa;
    ...
    public function altera($laboratorio) {
        try {
            $stmt = parent::prepare("UPDATE `laboratorio` SET `Tipo`=?,`Capacidade` = ?,`Nome` =?,`Sigla` = ?,`Resposta`=?, `Labensino`=?, `Nestacoes`=?, `Local`=?, `SisOperacional`=?,`CabEstruturado`=?,`Situacao`=?,`AnoDesativacao`=? WHERE CodLaboratorio=?");
            parent::beginTransaction();
            $stmt->bindValue(1, $laboratorio->getTipo()->getCodigo());
           // echo "area".$laboratorio->getArea();
            //$stmt->bindValue(2, $laboratorio->getArea());
            $stmt->bindValue(2, $laboratorio->getCapacidade());
            $stmt->bindValue(3, $laboratorio->getNome());
            $stmt->bindValue(4, $laboratorio->getSigla());
           // print "upda labresposta" . $laboratorio->getResposta() . "<br/>";
            $stmt->bindValue(5, $laboratorio->getResposta());
            //print "upda labensino" . $laboratorio->getLabensino() . "<br/>";
            $stmt->bindValue(6, $laboratorio->getLabensino());
            $stmt->bindValue(7, $laboratorio->getNestacoes());
            $stmt->bindValue(8, $laboratorio->getLocal());
            $stmt->bindValue(9, $laboratorio->getSo());
            $stmt->bindValue(10, $laboratorio->getCabo());
            $stmt->bindValue(11, $laboratorio->getSituacao());
            $stmt->bindValue(12, $laboratorio->getAnodesativacao());
            $stmt->bindValue(13, $laboratorio->getCodlaboratorio());
            $stmt->execute();
            parent::commit();
        } catch (PDOException $ex) {
            parent::rollback();
            print "Erro altera: Código:" . $ex->getCode() . "Mensagem" . $ex->getMessage();
        }
    }   
    ...
    public function buscaNomeLaboratorio($nome,$anobase) {
        try {
            $nome="%".$nome."%";
...
public function buscaLaboratorioPDF($codunidade,$anobase) {
 ...
 AND (l.`AnoDesativacao` IS NULL) ";
    ...
    public function areaAnoAnterior($codlab,$anoBase) {
    	try {
    		
    		$stmt = parent::prepare("SELECT area,max(ano) FROM `area_lab` WHERE CodLaboratorio=:lab AND ano<:ano GROUP BY area");
    		$stmt->execute(array(':lab' => $codlab,':ano'=>$anoBase));
    		return $stmt;
    	} catch (PDOException $ex) {
    		print "Erro: Código:" . $ex->getCode() . "Mensagem" . $ex->getMessage();
