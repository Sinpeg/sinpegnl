...
order by con.ordem asc";
	...
	//Buscar pendências na finalização do relatório
	public function buscarPendencias($codUnidade,$ano,$topico){
	    try {
	        parent::beginTransaction();
	        $stmt = parent::query("SELECT * FROM `raa_texto` WHERE codTopico = ".$topico." AND codUnidade = ".$codUnidade." AND ano = ".$ano."");
	        $stmt->execute();
	        parent::commit();
	        return $stmt;
	    } catch (PDOException $ex){
