...
function incluirTopicoNoTexto($rowsl,$topico,$cont,$codunidade,$anobase){
    ...
    foreach ($rowsl as $row) {
           ...
           if ($row['ordem']==10 && empty($row['codTopico'])){
               $ordemintroducao=$row['ordem']-1;
...
function subtopico($topico,$codunidade,$anobase,$nivel,$cont){
	foreach ($topico as $t){ 		
		$cont++;
		?>
		<h3 <?php  print " onclick=carregaModelo(".$t->getCodigo().")";?> > <?php // print $t->getNivel()." ".$t->getNome();?>
			<?php
				$exibirNivel = $nivel . "." . $cont;
				print $exibirNivel . " " . $t->getNome();
			?>
		</h3>	
		...
		if ($t->getSubtopicos()!=NULL){
			...
			$cont=subtopico($t->getSubtopicos(),$codunidade,$anobase,$nivel,$cont);
		}
	}//for
    return $cont;
}
...
function subtopicoArquivo($topico,$codunidade,$anobase,$nivelT){
	...
	foreach ($topico as $t){ 
		...
		if ($trows->rowCount()>0){
			...
			$sNivel++;
			$html .= '<br/><h3>'.$nivelT.'. '.$sNivel." ".$t->getNome().'</h3>';		    
		    //Carla =========================
			if ($t->getCodigo()==143){
			  			include_once 'inclusoesraa.php';
